...
ffmpeg_command = (
    ...
    f"[9:v]scale={video_width}:{video_height}[v9]; "
    f"[10:v]scale={video_width}:{video_height}[v10]; "
    f"[11:v]scale={video_width}:{video_height}[v11]; "
    f"[12:v]scale={video_width}:{video_height}[v12]; "
    f"[13:v]scale={video_width}:{video_height}[v13]; "
    f"[14:v]scale={video_width}:{video_height}[v14]; "
    f"[15:v]scale={video_width}:{video_height}[v15]; "
    f"[v0][v1][v2][v3]hstack=inputs=4[row1]; "
    f"[v4][v5][v6][v7]hstack=inputs=4[row2]; "
    f"[v8][v9][v10][v11]hstack=inputs=4[row3]; "
    f"[v12][v13][v14][v15]hstack=inputs=4[row4]; "
    f'[row1][row2][row3][row4]vstack=inputs=4[vstack]" '
    f'-map "[vstack]" '
